...
8. **When several sources cover the same story, report where they disagree.** Multiple independent outlets covering one development is itself a signal that the story matters, and the value of that coverage is the range of positions it exposes. Do not synthesize many sources into a single flattened consensus. Where the sourcing supports it, name who holds which position and what separates them: regulators, public-health officials, researchers, consumer advocates, industry operators, and trade associations frequently disagree about the same facts. Attribution rules apply unchanged, so a disagreement is reported as "X said A, while Y said B," never adjudicated by the publication. Where sources conflict on a factual point rather than an opinion, say so plainly ("accounts differ on whether the products were tested before the recall") instead of silently choosing one. A story covered by one source gets one perspective and should not be dressed up as a debate.

9. **Never treat "kratom supporters" and "7-OH supporters" as the same constituency.** This is the most commonly botched framing in kratom coverage and the publication corrects for it deliberately. Kratom proponents, including the major trade and advocacy organizations, frequently oppose concentrated and synthetic 7-hydroxymitragynine products, and have supported restrictions on them. A regulatory action against synthetic 7-OH is therefore not automatically a defeat for kratom advocates, and reporting that frames it that way is wrong on the facts. When a story involves 7-OH policy, establish where the kratom advocacy position actually sits rather than assuming it opposes the restriction, and name the organization and its stated stance where the sourcing supports it. Where advocates split among themselves, report the split. The default assumption of a single unified "pro-kratom" bloc is what this rule exists to prevent, and the divergence is often the most newsworthy part of the story.
